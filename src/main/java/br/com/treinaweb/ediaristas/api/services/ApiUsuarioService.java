...
public class ApiUsuarioService {
    ...
    private void verificarSenha(AtualizarUsuarioRequest request, Usuario usuarioLogado) {
        ...
        var senhaBD = usuarioLogado.getSenha();

        if (!passwordEncoder.matches(senhaRequest, senhaBD)) {
            var mensagem = "A senha informada está incorreta";
            var fieldError = new FieldError(request.getClass().getName(), "password", senhaRequest, false, null, null,
                    mensagem);
    ...
    private void atualizarInformacoesUsuarioLogado(AtualizarUsuarioRequest request, Usuario usuarioLogado) {
        usuarioLogado.setNomeCompleto(firstNonNull(request.getNomeCompleto(), usuarioLogado.getNomeCompleto()));
        usuarioLogado.setEmail(firstNonNull(request.getEmail(), usuarioLogado.getEmail()));
        usuarioLogado.setCpf(firstNonNull(request.getCpf(), usuarioLogado.getCpf()));
        usuarioLogado.setNascimento(firstNonNull(request.getNascimento(), usuarioLogado.getNascimento()));
        usuarioLogado.setTelefone(firstNonNull(request.getTelefone(), usuarioLogado.getTelefone()));
        usuarioLogado.setChavePix(firstNonNull(request.getChavePix(), usuarioLogado.getChavePix()));
    }
