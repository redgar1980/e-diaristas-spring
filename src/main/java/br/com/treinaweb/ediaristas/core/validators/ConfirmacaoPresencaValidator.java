...
public class ConfirmacaoPresencaValidator {
    ...
    private void validarStatus(Diaria diaria) {
        if (!diaria.isConfirmado()) {
            ...
            var fiedlError = new FieldError(diaria.getClass().getName(), "status", diaria.getStatus(), false, null,
                    null, mensagem);
            throw new ValidacaoException(mensagem, fiedlError);
        }
    ...
    private void validarDataAtendimento(Diaria diaria) {
        ...
        if (dataAtendimento.isAfter(hoje)) {
            ...
            var fiedlError = new FieldError(diaria.getClass().getName(), "dataAtendimento", diaria.getDataAtendimento(),
                    false,
                    null,
                    null, mensagem);
            throw new ValidacaoException(mensagem, fiedlError);
    ...
    private void validarDiarista(Diaria diaria) {
        if (diaria.getDiarista() == null) {
            ...
            var fiedlError = new FieldError(diaria.getClass().getName(), "diarista", diaria.getDiarista(), false,
                    null,
                    null, mensagem);
            throw new ValidacaoException(mensagem, fiedlError);
        }
