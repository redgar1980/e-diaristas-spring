...
public class GoogleMatrixService implements ConsultaDistanciaService {
  ...
  public DistanciaResponse calcularDistanciaEntreDoisCeps(String origem, String destino) {
    ...
    return distanciaMatrixToDistanciaResponse(distanceMatrix);
  }

  private DistanciaResponse distanciaMatrixToDistanciaResponse(DistanceMatrix distanceMatrix) {
    ...
    return DistanciaResponse.builder()
        ...
        .distanciaEmQuilometros(distanceMatrix.rows[0].elements[0].distance.inMeters / 1000.0)
        .build();
  }

  private DistanceMatrix getDistanceMatrix(String origem, String destino,
    ...
    try {
      ...
      throw new ConsultaDistanciaServiceException(exception.getLocalizedMessage());
    }
  }
  ...
  private void validarDistanceMatrix(DistanceMatrix distanceMatrix) {
    ...
    if (rows.length == 0) {
      throw new ConsultaDistanciaServiceException("Erro ao realizar consulta");
    }
  ...
  private void validarCep(String cep) {
    if (cep.length() != 8) {
      throw new IllegalArgumentException("O cep deve ter oito dígitos");
    }
